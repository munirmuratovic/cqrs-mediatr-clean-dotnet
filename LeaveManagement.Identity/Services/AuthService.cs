...
public class AuthService : IAuthService
{
    ...
    public async Task<AuthResponse> Login(AuthRequest request)
    {
        ...
        if (user == null)
        {
            throw new NotFoundException($"User with email {request.Email} not found.", request.Email);
        }
        ...
        if (!result.Succeeded)
        {
            throw new BadRequestException($"Credentials for email {request.Email} are not valid.");
        }
